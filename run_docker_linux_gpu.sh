...
if [ -f $configfile ]; then 
	...
	sudo docker run --rm --gpus all -it -e TRAIN_NAME=$container_name -e TRAIN_START_TIME="$(date '+%Y%m%d_%H:%M:%S')" $ports  -v $folder_path:/training/assets -v $(pwd)/trainings:/training/custom_training -v /etc/timezone:/etc/timezone:ro -v /etc/localtime:/etc/localtime:ro --name $container_name witwickey/yolo_training_automation:0.1 ;
else	
